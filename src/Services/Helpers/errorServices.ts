...
export const errorCheck = async (error: any) => {
    ...
    } else if (error?.message?.includes("Inability to pay some fees" || "account balance too low")) {
        store?.dispatch(setErrorMessage("Insufficient Balance For Gas Fee In Your Account"));
    } else if(error?.message?.includes("No response received from RPC endpoint in 60s")){
